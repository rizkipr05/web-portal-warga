...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['set_status'])) {
    ...
    $info = "Status user #$id diubah ke $status.";
  }
  if (isset($_POST['delete'])) {
    $id = (int)$_POST['id'];
    $pdo->prepare("DELETE FROM users WHERE id=?")->execute([$id]);
    $info = "User #$id dihapus.";
  }
}

// Query
$q = trim($_GET['q'] ?? '');
$sql = "SELECT id,name,nik,email,status,created_at FROM users WHERE 1";
$params=[];
if($q!==''){
  $sql.=" AND (name LIKE ? OR nik LIKE ? OR email LIKE ?)";
  $params=["%$q%","%$q%","%$q%"];
}
...
<head>
  <meta charset="utf-8">
  <title>Kelola Warga — Admin</title>
...
<div class="layout">
  ...
  <aside id="sb" class="sidebar">
    ...
    <nav class="nav-aside d-grid gap-1">
      ...
      <a class="active" href="manage_users.php"><i class="bi bi-people"></i> Kelola Warga</a>
  ...
  <main>
    ...
    <div class="topbar-wrap">
      <div class="topbar">
        <div class="d-flex align-items-center gap-2 gap-md-3">
          ...
          <div class="page-title me-auto">Kelola Warga</div>

          <form class="d-none d-md-flex align-items-center" method="get">
            <div class="input-group">
              <span class="input-group-text bg-white border-end-0"><i class="bi bi-search"></i></span>
              <input class="form-control searchbox border-start-0" name="q" placeholder="Cari nama/NIK/email…" value="<?= htmlspecialchars($q) ?>"/>
            </div>
          </form>
    ...
    <div class="content">
      ...
      <form class="d-flex d-md-none mb-3" method="get">
        <input class="form-control me-2" name="q" placeholder="Cari nama/NIK/email…" value="<?= htmlspecialchars($q) ?>">
        ...
        <div class="card-body">
          <div class="table-responsive">
            <table class="table align-middle">
              <thead>
                <tr>
                  <th>ID</th><th>Nama</th><th>NIK</th><th>Email</th><th>Status</th><th>Sejak</th><th style="width:220px">Aksi</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr>
                  ...
                  <td><code><?= htmlspecialchars($r['nik']) ?></code></td>
                  <td><?= htmlspecialchars($r['email']) ?></td>
                  <td>
                    <span class="badge badge-status <?= $r['status']==='active'?'active':'suspended' ?>">
                  ...
                  <td>
                    <form class="d-inline" method="post">
                      ...
                      <button name="set_status" class="btn btn-sm btn-outline-primary">
                        <i class="bi bi-toggle2-<?= $r['status']==='active'?'off':'on' ?>"></i>
                        <?= $r['status']==='active'?'Nonaktifkan':'Aktifkan' ?>
                      </button>
                    </form>
                    <form class="d-inline" method="post" onsubmit="return confirm('Hapus user ini?')">
                      <input type="hidden" name="id" value="<?= (int)$r['id'] ?>">
                      <button name="delete" class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button>
                    </form>
                  </td>
                </tr>
